...
const StatusPill = ({ status }) => {
  ...
  const tone = s.startsWith("CANCEL")
    ? map.CANCELLED
...
const KPI = ({ title, value, note, gradient }) => (
  <div className="rounded-xl border bg-white p-4 shadow-sm">
    <div className="flex items-center justify-between">
      ...
      <div
        className={`h-10 w-10 rounded-md ${gradient} flex items-center justify-center text-white`}
      >
        📊
      </div>
    </div>
...
const BookingsPage = () => {
  ...
  const getCustomerName = (booking) => {
    ...
    return (
      cached?.fullName || cached?.email || (uid != null ? `User #${uid}` : "-")
    );
  };
    ...
    return (bookings || []).filter((b) => {
      ...
      const src = fallback
        ? {
            ...
            ...fallback,
            passengers:
              Array.isArray(fallback?.passengers) &&
              fallback.passengers.length > 0
                ? fallback.passengers
                : b?.passengers,
          }
        : b;
      const hay =
        `${getCustomerName(src)} ${getCustomerPhone(src)} ${getTourTitle(src)} ${src.bookingCode || ""}`.toLowerCase();
  ...
  const stats = useMemo(() => {
    ...
    const cancelled = all.filter((b) =>
      normalizeStatus(b.status).startsWith("CANCEL"),
    ).length;
        ...
        const results = await Promise.all(
          candidates.map(async (b) => {
            ...
            if (res && res.status === 200)
              return { code, detail: res.data || res };
      ...
      new Set(
        pool
          ...
          .filter((id) => {
            ...
            if (!b) return false;
            const hasName =
              getCustomerNameRaw(b) && getCustomerNameRaw(b) !== "-";
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      if (res && res.status === 200) {
        ...
        setToast({
          type: "error",
          message: res?.message || "Không tải được bookings",
        });
  ...
  const fetchBookingDetail = async (code) => {
    ...
    try {
      ...
      if (res && res.status === 200) {
        ...
        setDetail((prev) => {
          const prevPassengers =
            Array.isArray(prev?.passengers) && prev.passengers.length > 0
              ? prev.passengers
              : null;
        ...
        setDetailByCode((prev) => {
          ...
          return {
            ...
            [String(code)]: {
              ...(existing || {}),
              ...(data || {}),
              ...(existingPassengers && !nextPassengers
        ...
        setDetail({
          error: res?.message || "Không lấy được chi tiết",
          bookingCode: code,
        });
      }
    } catch (err) {
      ...
      setDetail({
        error: err?.message || "Lỗi khi lấy chi tiết",
        bookingCode: code,
      });
    }
  };
      ...
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-100">
                    ...
                    >
                      ...
                      <td className="p-3 font-semibold text-sky-600">
                        {b.totalAmount || b.amount
                          ? (b.totalAmount || b.amount).toString()
                          : "-"}
                      </td>
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                              ...
                              if (code) {
                                setDetailByCode((prev) => {
                                  ...
                                  const mergedPassengers =
                                    existingPassengers || rowPassengers;
                                  return {
                                    ...prev,
                                    [code]: {
                                      ...(b || {}),
                          ...
                          <button className="rounded-full border px-2 py-1 text-xs text-rose-600 hover:bg-rose-50">
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="text-sm text-slate-500">
              Hiển thị {paged.length} trên {filtered.length} bookings
            </div>
              ...
              <div className="flex items-center justify-between px-5 py-4 rounded-t-2xl bg-linear-to-r from-sky-600 to-indigo-600 text-white">
                <div>
                  ...
                  <h3 className="text-base font-semibold">
                    {detailLoading ? "Đang tải..." : detail?.bookingCode || "-"}
                  </h3>
                  <div className="text-xs text-white/80">
                    {getTourTitle(detail) !== "-" ? getTourTitle(detail) : ""}
                  </div>
                </div>
                ...
                {detail?.error ? (
                  <div className="p-4 rounded-md border bg-rose-50 text-rose-700">
                    ...
                    <div className="mt-2 flex gap-2">
                      <button
                        onClick={() => fetchBookingDetail(lastDetailCode)}
                        ...
                        <div>
                          ...
                          <span className="font-medium">
                            {detail?.email ||
                              ...
                              detail?.passengers?.[0]?.email ||
                              (getBookingUserId(detail) != null
                                ? userById[String(getBookingUserId(detail))]
                                    ?.email || "-"
                                : "-")}
                          </span>
                        </div>
                      ...
                      detail.passengers.length > 0 && (
                        <div className="mt-6">
                          ...
                          <div className="space-y-2">
                            {detail.passengers.map((p, idx) => (
                              <div key={idx} className="rounded-md border p-3">
                                ...
                                <div className="text-xs text-slate-500">
                                  {p.idCardNumber || "No ID"} • {p.ageGroup} •{" "}
                                  {p.gender} • {p.dob || "-"}
                                </div>
                              </div>
                            ))}
                          ...
                          <div className="mt-2 text-xs text-slate-500">
                            Chi tiết passenger nằm ở menu Passengers.
                          </div>
                        </div>
                      )}
