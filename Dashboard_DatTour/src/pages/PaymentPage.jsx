...
const PaymentStatusPill = ({ status }) => {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${map[s] || 'bg-slate-50 text-slate-700'}`}>
      {status || '-'}
    </span>
  );
};
...
export const PaymentPage = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      if (res && res.status === 200) {
        ...
        setItems([]);
        setToast({ type: 'error', message: res?.message || 'Không tải được payment report' });
  ...
  const stats = useMemo(() => {
    const list = items || [];
    const success = list.filter((x) => normalizeStatus(x.paymentStatus) === 'SUCCESS').length;
    const pending = list.filter((x) => normalizeStatus(x.paymentStatus) === 'PENDING').length;
    const failed = list.filter((x) => ['FAILED', 'CANCELLED'].includes(normalizeStatus(x.paymentStatus))).length;
    const totalRevenue = list
      .filter((x) => normalizeStatus(x.paymentStatus) === 'SUCCESS')
      .reduce((sum, x) => sum + (typeof x.amount === 'number' ? x.amount : Number(x.amount) || 0), 0);
  ...
  const filtered = useMemo(() => {
    ...
    return (items || []).filter((x) => {
      ...
      if (!q) return true;
      const bookingCode = x?.bookingDetails?.bookingCode || x?.transactionId || '';
      const customerName = x?.customerDetails?.fullName || x?.customerDetails?.email || '';
      const hay = `${x.transactionId || ''} ${bookingCode} ${customerName} ${x.paymentMethodName || ''}`.toLowerCase();
      ...
      <div className="max-w-6xl mx-auto space-y-4">
        <MiniStatSquares
          items={[
            ...
            { label: 'Tổng thu', value: formatMoney(stats.totalRevenue), badge: 'VNĐ' },
          ]}
        />

        <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-100">
          <div className="mb-3 text-sm text-slate-500">{loading ? 'Đang tải payment report...' : ''}</div>
            ...
            <table className="w-full table-auto text-sm">
              ...
              <tbody>
                {paged.length === 0 ? (
                  ...
                  return (
                    <tr key={x.id || x.transactionId} className="border-b last:border-0 odd:bg-white even:bg-slate-50/50 hover:bg-sky-50/40 transition">
                      <td className="p-3">
                        <div className="font-semibold">{x.transactionId || '-'}</div>
                        <div className="text-xs text-slate-500">{bookingCode ? `Booking: ${bookingCode}` : ''}</div>
                      </td>
                      <td className="p-3">
                        <div className="font-semibold">{customer?.fullName || customer?.email || '-'}</div>
                        <div className="text-xs text-slate-500">{customer?.phone || '-'}</div>
                      </td>
                      <td className="p-3 font-semibold text-sky-600">{formatMoney(x.amount)}</td>
                      <td className="p-3 text-slate-600">{x.paymentMethodName || '-'}</td>
                      <td className="p-3 text-slate-600">{formatDateTime(x.paidAt)}</td>
                      <td className="p-3"><PaymentStatusPill status={x.paymentStatus} /></td>
                      <td className="p-3">
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="text-sm text-slate-500">Hiển thị {paged.length} trên {filtered.length} giao dịch</div>
          ...
          <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 px-4 py-6 overflow-y-auto">
            <div className="w-full max-w-4xl rounded-2xl bg-white shadow-xl max-h-[calc(100vh-3rem)] flex flex-col">
              <div className="flex items-center justify-between px-5 py-4 rounded-t-2xl bg-linear-to-r from-sky-600 to-indigo-600 text-white">
                <div>
                  <div className="text-xs uppercase tracking-wide text-white/80">Chi tiết payment</div>
                  <h3 className="text-base font-semibold">{detail?.transactionId || '-'}</h3>
                  <div className="text-xs text-white/80">{detail?.bookingDetails?.tourTitle || ''}</div>
                </div>
              ...
              <div className="p-5 overflow-y-auto overscroll-contain">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <div className="text-xs text-slate-500">Trạng thái payment</div>
                    <div className="font-semibold"><PaymentStatusPill status={detail?.paymentStatus} /></div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-500">Phương thức</div>
                    <div className="font-semibold">{detail?.paymentMethodName || detail?.gateway || '-'}</div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-500">Số tiền</div>
                    <div className="font-semibold text-sky-700">{formatMoney(detail?.amount)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-500">Paid at</div>
                    <div className="font-semibold">{formatDateTime(detail?.paidAt)}</div>
                  </div>
                </div>
                ...
                <div className="mt-4 flex items-center gap-2">
                  {detail?.paymentUrl ? (
                    ...
                    <button disabled className="rounded-md bg-slate-200 px-3 py-2 text-sm text-slate-500">Không có paymentUrl</button>
                  )}
                  {detailLoading ? (
                    <div className="text-sm text-slate-500">Đang lấy paymentUrl...</div>
                  ) : null}
                </div>
                ...
                {detail?.bookingDetails && (
                  <div className="mt-5 rounded-xl border border-slate-100 bg-white p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      <div><span className="text-slate-500 mr-2">Booking code:</span> <span className="font-medium">{detail.bookingDetails.bookingCode || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">Trạng thái:</span> <span className="font-medium">{detail.bookingDetails.status || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">Tổng tiền:</span> <span className="font-medium">{formatMoney(detail.bookingDetails.totalAmount)}</span></div>
                      <div><span className="text-slate-500 mr-2">Ngày tạo:</span> <span className="font-medium">{formatDateTime(detail.bookingDetails.createdAt)}</span></div>
                      <div className="md:col-span-2"><span className="text-slate-500 mr-2">Tour:</span> <span className="font-medium">{detail.bookingDetails.tourTitle || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">Ngày đi:</span> <span className="font-medium">{formatDateTime(detail.bookingDetails.startDate)}</span></div>
                      <div><span className="text-slate-500 mr-2">User ID:</span> <span className="font-medium">{detail.bookingDetails.userId ?? '-'}</span></div>
                    </div>
                    ...
                    {detail.bookingDetails.priceDetail && (
                      <div className="mt-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-slate-700">
                          {Object.entries(detail.bookingDetails.priceDetail).map(([k, v]) => (
                            <div key={k}><span className="text-slate-500 mr-2">{PRICE_LABELS[k] || k}:</span> <span className="font-medium">{formatMoney(v)}</span></div>
                          ))}
                        </div>
                      </div>
                    )}

                    {Array.isArray(detail.bookingDetails.passengers) && detail.bookingDetails.passengers.length > 0 && (
                      <div className="mt-5">
                ...
                {detail?.customerDetails && (
                  <div className="mt-5 rounded-xl border border-slate-100 bg-slate-50 p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-slate-700">
                      <div><span className="text-slate-500 mr-2">Họ tên:</span> <span className="font-medium">{detail.customerDetails.fullName || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">SĐT:</span> <span className="font-medium">{detail.customerDetails.phone || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">Email:</span> <span className="font-medium">{detail.customerDetails.email || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">Trạng thái:</span> <span className="font-medium">{detail.customerDetails.status || '-'}</span></div>
                      <div className="md:col-span-2"><span className="text-slate-500 mr-2">Địa chỉ:</span> <span className="font-medium">{detail.customerDetails.address || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">Ngày sinh:</span> <span className="font-medium">{detail.customerDetails.dob || '-'}</span></div>
                      <div><span className="text-slate-500 mr-2">Giới tính:</span> <span className="font-medium">{detail.customerDetails.gender || '-'}</span></div>
                      <div className="md:col-span-2"><span className="text-slate-500 mr-2">Roles:</span> <span className="font-medium">{Array.isArray(detail.customerDetails.roles) ? detail.customerDetails.roles.join(', ') : '-'}</span></div>
                    </div>
                  </div>
                )}

                <div className="mt-6 flex justify-end">
                  <button onClick={() => setShowDetailModal(false)} className="rounded-md border px-3 py-2 text-sm">Đóng</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {toast && (
          <div className={`fixed right-6 bottom-6 z-60 rounded-md px-4 py-3 shadow-md ${toast.type === 'success' ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'}`}>
            <div className="flex items-center gap-3">
              ...
              {toast.type === 'error' ? (
                <button onClick={fetchReport} className="rounded-md bg-white/20 px-2 py-1 text-white text-xs">Retry</button>
              ) : null}
            </div>
          </div>
        )}
