...
export const UsersPage = () => {
  ...
  const stats = useMemo(
    () => ({
      total: users.length,
      active: users.filter((u) => (u.status || "").toUpperCase() === "ACTIVE")
        .length,
  ...
  const openUser = async (uOrId, onlyView = false) => {
    ...
    try {
      ...
      if (res && res.status === 200 && res.data) {
        ...
        setToast({
          type: "error",
          message: res?.message || "Không thể lấy thông tin người dùng",
        });
      ...
      setToast({
        type: "error",
        message: err?.message || "Lỗi khi lấy dữ liệu",
      });
      setTimeout(() => setToast(null), 4000);
    } finally {
  ...
  const handleDelete = async (id) => {
    setConfirm({
      ...
      message:
        "Bạn có chắc chắn muốn xoá người dùng này? Hành động không thể hoàn tác.",
      async onConfirm() {
        setConfirm(null);
        setModalLoading(true);
        try {
          ...
          if (res && res.status === 200) {
            ...
            setToast({
              type: "error",
              message: res?.message || "Xoá thất bại",
            });
            setTimeout(() => setToast(null), 4000);
          }
        } catch (err) {
  ...
  const handleAssignRoles = async () => {
    if (!selected) return;
    setModalLoading(true);
    const res = await authApi.assignRoles({
      userId: selected.id,
      roleNames: selected.roles || [],
    });
    setModalLoading(false);
    if (res && res.status === 200) {
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select
                ...
                className="rounded-lg border bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
              >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={fetchUsers}
                className="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-3 py-2 text-sm font-medium text-white hover:bg-sky-700 transition"
              >
                ...
                {loading ? (
                  ...
                  paged.map((u) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold ${u.status === "ACTIVE" ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100" : "bg-slate-100 text-slate-500 ring-1 ring-slate-200"}`}
                        >
                          {u.status}
                        </span>
                      </td>
                      <td className="p-3">
                        {(u.roles || []).map((r) => (
                          <span
                            key={r}
                            className="mr-1.5 inline-block rounded-full bg-slate-100 px-2.5 py-0.5 text-xs text-slate-600 ring-1 ring-slate-200"
                          >
                            {r}
                          </span>
                        ))}
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => openUser(u, true)}
                            className="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs bg-white hover:bg-slate-50"
                          >
                            <Icon name="eye" className="h-4 w-4" />
                            Xem
                          </button>
                          <button
                            onClick={() => openUser(u, false)}
                            className="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs bg-white hover:bg-slate-50"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs text-rose-600 hover:bg-rose-50"
                          >
              ...
              <div className="flex items-center justify-between px-5 py-4 rounded-t-2xl bg-gradient-to-r from-sky-600 to-indigo-600 text-white">
                <div>
                  ...
                  <h3 className="text-base font-semibold">
                    {modalLoading
                      ? "Đang tải..."
                      : `#${selected?.id || ""} • ${selected?.fullName || "(No name)"}`}
                  </h3>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`px-3 py-1 rounded-md text-sm ${modalTab === "info" ? "bg-white text-sky-700" : "bg-white/20 text-white"}`}
                    >
                      Thông tin
                    </button>
                    <button
                      ...
                      className={`px-3 py-1 rounded-md text-sm ${modalTab === "roles" ? "bg-white text-sky-700" : "bg-white/20 text-white"}`}
                    >
                      Phân quyền
                    </button>
                  </div>
                    ...
                    {modalTab === "info" && (
                      <>
                        ...
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                          ...
                          <input
                            readOnly={viewOnly}
                            value={selected.currentPoints ?? 0}
                            onChange={(e) =>
                              setSelected({
                                ...selected,
                                currentPoints: Number(e.target.value),
                              })
                            }
                            type="number"
                            className="rounded-lg border bg-slate-50 px-3 py-2 text-sm"
                          ...
                          {["ADMIN", "USER", "CUSTOMER"].map((r) => (
                            ...
                            >
                              <input
                                disabled={viewOnly}
                                type="checkbox"
                                checked={(selected.roles || []).includes(r)}
                                onChange={() => {
                                  const has = (selected.roles || []).includes(
                                    r,
                                  );
                                  setSelected({
                                    ...selected,
